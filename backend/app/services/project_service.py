...
async def sync_projects_from_github(db: AsyncSession, github_username: str) -> List[ProjectRead]:
    ...
    if not github_repos:
        ...
        # Fetch existing projects from DB to return them if GitHub fetch fails or returns empty
        db_projects = await crud_project.get_projects(db=db)
        return [ProjectRead.model_validate(p) for p in db_projects]
    ...
    for repo in github_repos:
        ...
        if existing_project_by_title:
            ...
            # Optionally, add it to synced_projects if you want to return existing ones that were skipped
            # synced_projects.append(ProjectRead.model_validate(existing_project_by_title))
            already_exists_count +=1 # Count as already existing
            continue

        # Then, as a fallback or secondary check, see if it exists by URL (current behavior)
        # This might be redundant if title is the primary unique factor for *new* entries from GitHub
        ...
        if existing_project_by_url:
            logger.info(f"Project with GitHub URL '{repo.html_url}' already exists (title: {existing_project_by_url.title}). Skipping to avoid conflict based on URL.")
            synced_projects.append(ProjectRead.model_validate(existing_project_by_url))
    ...
    # After syncing, re-fetch all projects to ensure the list is comprehensive
    all_db_projects = await crud_project.get_projects(db=db) # Assuming get_projects returns all
    return [ProjectRead.model_validate(p) for p in all_db_projects]
