...
async def get_or_create_superuser(db: "AsyncSession", superuser_data: Dict[str, Any]) -> User:
    ...
    user_obj = await db.execute(select(User).where(User.email == email))
    ...
    user_in = UserSchema(**superuser_data)
    created_user = await crud.user.create_superuser(db, obj_in=user_in, password=settings.FIRST_SUPERUSER_PASSWORD)
    return created_user
...
async def sync_model(db: "AsyncSession", model_name: str, data_list: List[Dict[str, Any]]):
    ...
    items_to_add = [Model(**item_data) for item_id, item_data in data_by_id.items() if item_id not in existing_items]
    if items_to_add:
...
async def main():
    ...
    parser.add_argument(
        "mode",
        choices=["sync", "dump", "fix-slugs", "clean-news"],
        help=(
