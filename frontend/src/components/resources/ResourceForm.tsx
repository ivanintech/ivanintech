...
const ResourceForm: React.FC<ResourceFormProps> = ({ onResourceAdded }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!url.trim()) {
      setFormError('The URL is required.');
      toast.error('The resource URL is required.');
      return;
    }
    try {
      new URL(url);
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(responseData.detail || `Error ${response.status} when adding the resource`);
      }
      
      const newResource: ResourceLink = responseData;
      toast.success('Resource submitted! It has been added to the list.');
      setUrl(''); 
      onResourceAdded(newResource); // Devolver el nuevo recurso
      
    } catch (err) {
      const error = err as Error & { response?: { data?: { detail?: string } } };
      setFormError(error.message || 'Could not add the resource.');
      toast.error(error.message || 'Could not add the resource.');
    }
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="grid grid-cols-1 gap-y-5">
          <div>
            ...
            <input 
              ...
              placeholder="https://example.com/resource-about-ai"
            />
        ...
        {!token && (
          <p className="mt-3 text-xs text-center text-gray-500 dark:text-gray-400">
            You must be logged in to share a resource.
          </p>
        )}
      </form>
    </div>
